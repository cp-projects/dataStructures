...
typedef struct dbl_op_flt{
    float* val;
    dbl_op_flt_t *next, *prev;
}dbl_op_flt_t;
...
void clear_list_dbl_op_flt(doubly_t* list){
     ...
     dbl_op_flt_t* pos = list->head->next;
     dbl_op_flt_t* swap = list->head->next;
     
     dbl_op_flt_t* head = list->head;
     dbl_op_flt_t* tail = list->tail;
...
void destroy_list_dbl_op_flt(doubly_t* list){
    clear_list_dbl_op_flt(list);
    free((void*)list);
}


void push_dbl_op_flt(doubly_t* list, const float val){

    //create node
    dbl_op_flt_t* new_node = (dbl_op_flt_t*) malloc(sizeof(dbl_op_flt_t));
...
float pop_dbl_op_flt(doubly_t* list){

    dbl_op_flt_t* remove_node = list->head; 
...
void enqueue_dbl_op_flt(doubly_t* list, const float val){
...
float dequeue_dbl_op_flt(doubly_t* list){

    dbl_op_flt_t* remove_node = list->tail;
    float val = *(remove_node->val);
...
void insert_at_head_dbl_op_flt(doubly_t* list, const float val){
...
float remove_at_head_dbl_op_flt(doubly_t* list){

    if(list->len == 0)
        return -1;

    dbl_op_flt_t* remove_node;
...
void insert_at_tail_dbl_op_flt(doubly_t* list, const float val){

    dbl_op_flt_t* old_tail = list->tail;
...
float remove_at_tail_dbl_op_flt(doubly_t* list){
...
void insert_after_index_dbl_op_flt(doubly_t* list, float val, int index){

    if(index == 0){
        insert_at_head_dbl_op_flt(list, val);
        return;
       }
	...
	return;
        ...
        insert_at_tail_dbl_op_flt(list, val);
	return;
       }

    dbl_op_flt_t* old_index = itr_forward_dbl_op_flt(list, 0, index, 0, 0, 1);
    dbl_op_flt_t* next_index = old_index->next;
...
float delete_at_index_dbl_op_flt(doubly_t* list, int index){
    ...
    if(index == 0)
        return remove_at_head_dbl_op_flt(list);
    ...
    if(index == list->len-1)
        return remove_at_tail_dbl_op_flt(list);
...
void insert_after_val_dbl_op_flt(doubly_t* list, float newVal, float testVal){
    ...
    if(list->len == 0){
        insert_at_head_dbl_op_flt(list, newVal);
	return;
       }

    else if(list->len == 1){
        if(*(list->head->val) == testVal)
	    insert_at_tail_dbl_op_flt(list, newVal);
	return;
       }
    
    dbl_op_flt_t* old_val = itr_forward_dbl_op_flt(list, testVal, 0, 0, 1, 0);
    ...
    dbl_op_flt_t* next_index = old_val->next;
