...
typedef struct dbl_op_int{
    int* val;
    dbl_op_int_t *next, *prev;
}dbl_op_int_t;
...
void clear_list_dbl_op_int(doubly_t* list){
     ...
     dbl_op_int_t* pos = list->head->next;
     dbl_op_int_t* swap = list->head->next;
     
     dbl_op_int_t* head = list->head;
     dbl_op_int_t* tail = list->tail;
...
void destroy_list_dbl_op_int(doubly_t* list){
    clear_list_dbl_op_int(list);
    free((void*)list);
}


void push_dbl_op_int(doubly_t* list, const int val){

    //create node
    dbl_op_int_t* new_node = (dbl_op_int_t*) malloc(sizeof(dbl_op_int_t));
...
int pop_dbl_op_int(doubly_t* list){

    dbl_op_int_t* remove_node = list->head; 
...
void enqueue_dbl_op_int(doubly_t* list, const int val){
...
int dequeue_dbl_op_int(doubly_t* list){

    dbl_op_int_t* remove_node = list->tail;
    int val = *(remove_node->val);
...
void insert_at_head_dbl_op_int(doubly_t* list, const int val){
...
int remove_at_head_dbl_op_int(doubly_t* list){

    if(list->len == 0)
        return -1;

    dbl_op_int_t* remove_node;
...
void insert_at_tail_dbl_op_int(doubly_t* list, const int val){

    dbl_op_int_t* old_tail = list->tail;
...
int remove_at_tail_dbl_op_int(doubly_t* list){
...
void insert_after_index_dbl_op_int(doubly_t* list, int val, int index){

    if(index == 0){
        insert_at_head_dbl_op_int(list, val);
        return;
       }
	...
	return;
        ...
        insert_at_tail_dbl_op_int(list, val);
	return;
       }

    dbl_op_int_t* old_index = itr_forward_dbl_op_int(list, 0, index, 0, 0, 1);
    dbl_op_int_t* next_index = old_index->next;
...
int delete_at_index_dbl_op_int(doubly_t* list, int index){
    ...
    if(index == 0)
        return remove_at_head_dbl_op_int(list);
    ...
    if(index == list->len-1)
        return remove_at_tail_dbl_op_int(list);
...
void insert_after_val_dbl_op_int(doubly_t* list, int newVal, int testVal){
    ...
    if(list->len == 0){
        insert_at_head_dbl_op_int(list, newVal);
	return;
       }

    else if(list->len == 1){
        if(*(list->head->val) == testVal)
	    insert_at_tail_dbl_op_int(list, newVal);
	return;
       }
    
    dbl_op_int_t* old_val = itr_forward_dbl_op_int(list, testVal, 0, 0, 1, 0);
    ...
    dbl_op_int_t* next_index = old_val->next;
